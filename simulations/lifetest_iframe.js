...
let simulation_settings = {
  ...
  particle_quantity: (/Android|webOS|iPhone|iPad|iPod|BlackBerry|BB|PlayBook|IEMobile|Windows Phone|Kindle|Silk|Opera Mini/i.test(navigator.userAgent)) ? 32 : 90,
  friction_override: null,

  simulation_area: { // simulation_settings.simulation_area

    width: 420, //window.innerWidth, // p is not defined. How to get window width outside of p5?
    ...
    dish_radius: 200, // applies to dish types if set
...
const sketch = function(p) {
  if (general_settings.visualize_trajectory === 'rand') {
    general_settings.visualize_trajectory = p.round(p.random(true,false))
  }
  if (general_settings.visualize_relationships === 'rand') {
    general_settings.visualize_relationships = p.round(p.random(true,false))
  }
  let types_input = {
    'atom (dense)': {
      color: [255, 0, 128],
  ...
  function textInformation(bottom_offset, ...info) {
    // found the issue
    let space_unit = 420/info.length;
    for (let i = 0; i < info.length; i++) {
      p.textAlign(p.CENTER)
      p.text( info[i], (space_unit * i) + space_unit / 2, 340-bottom_offset )
    }
  }
  ...
  function cameraControls() {
    ...
    if (p.mouseIsPressed && p.mouseButton === p.LEFT || p.mouseIsPressed && p.mouseButton === p.CENTER) {
  ...
  p.draw = function() {
    ...
    p.translate(
      ( 420/2 + (camera.zoom * camera.x) ),
      ( 340/2 + (camera.zoom * camera.y) )
    );
    ...
    //   p.fill(255)
    //   p.stroke(0x11,0x11,0x13);p.strokeWeight(20)
    //   p.textAlign(p.CENTER, p.CENTER)
    // p.textFont('fail', 60)
    //   p.text('Engine', 278, 170)
    //   p.textFont('Iosevka heavy', 60)
    //   p.text('Life', 110, 170)
	};
    ...
    if ( p.mouseButton === p.RIGHT) {
      ...
      particles.push(
        new Particle(
          ...
          new p5.Vector(((p.mouseX-420/2)/camera.zoom-camera.x) + p.random(0,1)/10000,
          ((p.mouseY-340/2)/camera.zoom-camera.y) + p.random(0,1)/10000),
